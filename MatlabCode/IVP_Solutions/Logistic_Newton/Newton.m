function [x1, num_itr, converged] = Newton(f, Df, x0, max_itr, err_tol)
  x1 = x0; % Initialize x1 as the initial guess x0

  f_x1 = f(x1);           % Compute f at the initial guess
  ...
  % If Df is not provided, define it using finite differences
  if isempty(Df)
    h = 1.0e-7;
    Df = @(x) JacobianFiniteDifferences(f, x, h);
  end
